...
export type GetProvidedDep<T> = T extends (...args: any[]) => infer R ? R : T extends Observable<infer R> ? R : T extends TokenDepSpec<infer X, infer XX> ? T['optional'] extends true ? XX | undefined : XX : T extends PackageDepSpec<infer T, infer TT> ? PackageType<T> : T;
...
export type DepTokenProvider<DT extends DepType, TT> = {
    token: DT;
    value?: TT;
    provide?: (input: Record<string, any>) => TT | Promise<TT>;
};
...
type DepTypeBase = TokenDepSpec<any, any> | InterfaceDepSpec<any> | MachineDepSpec<any> | PackageDepSpec<any, any>;
export type DepTypeWithPrivate = {
    private?: boolean;
};
export type DepType = DepTypeBase & DepTypeWithPrivate;
export type DepValueType<T extends DepType> = GetProvidedDep<T>;
export {};
//# sourceMappingURL=TokenDepSpec.d.ts.map