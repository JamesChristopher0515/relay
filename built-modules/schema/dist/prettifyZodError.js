import { get } from '@bbuild/dash';
import { colorize } from '@bbuild/format-terminal';
export function prettifyZodError(error, input, name) {
    let prettyError = '';
    ...
    error.issues.forEach((issue) => {
        ...
        //     break
        // }
        prettyError += `${colorize.red(`${issue.code} "${issue.message}"`)} ${colorize.blue('@' + (name ? name + '.' : '') + issue.path.join('.'))} \n`;
        if (input) {
            prettyError += colorize.yellow(`Received: ${JSON.stringify(get(input, issue.path))} of type ${typeof get(input, issue.path)}\n`);
        }
        // prettyError += '\n'
    });
    return prettyError;
