...
export function getModelTokens(model) {
...
export const OpenAIStream = async (messages, settings = {}) => {
    const model = settings?.model ?? Model.GPT4;
    const inputTokens = tokenizer.encodeChat(messages, model.includes('gpt-3.5-turbo') ? 'gpt-3.5-turbo' : 'gpt-4').length;
