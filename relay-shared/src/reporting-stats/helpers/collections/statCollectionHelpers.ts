...
export function getStatCollectionHelper(
  collectionType: keyof typeof statCollectionHelpers
): typeof statCollectionHelpers[typeof collectionType] {
  const helper = statCollectionHelpers[collectionType]
  invariant(
    helper,
    `getStatCollectionHelper: no helper for collection type ${collectionType}`
  )
  return helper
}
