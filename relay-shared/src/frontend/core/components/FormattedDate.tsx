...
export function safeFormat(formatStr: string, date?: Date, fallback = '') {
  try {
    if (date) {
...
export function getTimeTo(d: Date) {
  const now = new Date()
  ...
  if (isNegative) {
    return `${Math.abs(Object.values(largest)[0])} ${
      Object.keys(largest)[0]
    }${plural} ago`
  } else {
    return `in ${Math.abs(Object.values(largest)[0])} ${
      Object.keys(largest)[0]
    }${plural}`
  }
}
...
export function omitYearForLast12Months(date: Date, format: string) {
  const sameYear = isSameYear(new Date(), date)
  return sameYear ? format.replace(/\/?y+/, '').replace(',', '') : format
}
export function formatDate(date: Date, format: string, smartStuff = true) {
