...
export default function useAssignedJourneyHistory(assignedJourneyId: string) {
  ...
  if (assignedJourney) {
    ...
    const { groupedByMilestoneChanges, sortedNewestFirst } =
      groupClientStopsByMilestoneChange({
        journey,
        clientStops: stops,
      })

    return groupedByMilestoneChanges.flatMap(
      (milestoneStops, milestoneChangeIndex, arr) => {
        const firstClientStop = milestoneStops[0]
        const [milestone, milestoneIndex] = findId(
          journey.milestones,
          firstClientStop.milestone
        )
        if (!milestone) {
