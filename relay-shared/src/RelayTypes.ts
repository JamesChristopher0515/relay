...
import {
  ClientInsightsWidgetSchemas,
  ClientInsightsWidgetTypes,
  ClientInsightsWidgetTypeStr,
  JourneyStopTypes,
  WorksheetStopTypes,
} from './RelaySchema'
import { Role } from './users/helpers/roles'

export function fakeType<T = any>() {
  return {} as any as T
}
...
export type NewDocument<T> = {
  ...
  | 'createdAt'
  | 'updatedAt'
  | 'creator' /* auto assigned on backend */
>
...
export interface Practitioner extends User {
  ...
  schedule: {
    days: { start: Date; end: Date; enabled: boolean }[]
  }
...
export type Result = MongoDocument &
  Timestamps & {
    type: string
    value?: number
  }
...
export type Notification = MongoDocument &
  Timestamps & {
    type: string
    value?: any
    read: boolean
...
export type QuestionnaireResult = MongoDocument &
  Timestamps & {
    questionnaire: Id
    client: Id
    todo: Id
...
export type QuestionnaireResultAnswer = { question: Id; value: number }
...
export interface ClientInsightsPresetWidgetBase<
  Type extends ClientInsightsWidgetTypeStr,
  DataType = z.infer<ReturnType<typeof ClientInsightsWidgetSchemas>[Type]>
> extends MongoDocument {
  data: DataType
  type: Type
  location: 'main' | 'sidebar' | 'timeline'
}
export interface HowImFeelingWidget
  extends ClientInsightsPresetWidgetBase<ClientInsightsWidgetTypes.HowImFeeling> {}
export interface QuestionnaireResultWidget
  extends ClientInsightsPresetWidgetBase<ClientInsightsWidgetTypes.QuestionnaireResult> {}
export interface CheckInWidget
  extends ClientInsightsPresetWidgetBase<ClientInsightsWidgetTypes.CheckIn> {}
export interface TodoWidget
  extends ClientInsightsPresetWidgetBase<ClientInsightsWidgetTypes.Todo> {}

export interface HealthWidget
  extends ClientInsightsPresetWidgetBase<ClientInsightsWidgetTypes.Health> {}
export interface WordCloudWidget
  extends ClientInsightsPresetWidgetBase<ClientInsightsWidgetTypes.WordCloud> {}
export interface JourneyWidget
  extends ClientInsightsPresetWidgetBase<ClientInsightsWidgetTypes.Journey> {}
export interface RelayWidget
  extends ClientInsightsPresetWidgetBase<ClientInsightsWidgetTypes.Relay> {}
export interface GraphWidget
  extends ClientInsightsPresetWidgetBase<ClientInsightsWidgetTypes.Graph> {}
export type ClientInsightsPresetWidget<
  T extends ClientInsightsWidgetTypeStr | unknown = unknown
> = T extends ClientInsightsWidgetTypeStr
  ? ClientInsightsPresetWidgetBase<T>
  :
      | HowImFeelingWidget
      | QuestionnaireResultWidget
      | CheckInWidget
      | TodoWidget
      | WordCloudWidget
      | JourneyWidget
...
// TODO extract from worksheet item schemas
