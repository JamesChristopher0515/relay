...
export default makeController(function OnboardingController(
  ...
  const [conversationManager, setConversationManager] =
    useState<OnboardingConversationManager | null>(null)

  useEffect(() => {
    if (!isLoading) {
      const assignedResources = todos.map((todo) => todo.resource)
      const conversation = new OnboardingConversationManager(
        assignedResources,
        wrappedAxios
      )
