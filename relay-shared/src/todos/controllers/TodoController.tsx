...
export default makeController(function TodoController({
  ...
  const doc = _doc as ClientTodo & { onClick: any }
  ...
  const action = async () => {
    ...
    if (doc.appointment) {
      ...
      if (!canJoin) {
        const inPast =
          new Date(doc.appointment.date).getTime() < new Date().getTime()
        Toast.show(
          inPast
            ? `This appointment can no longer be joined`
            : `This appointment doesn't start until ${format(
                new Date(doc.appointment.date),
                'H:mm a'
              )}`
        )
        return
    ...
    if (doc.onClick) {
      ...
      const openable =
        doc.assignedResource?.type === 'content' ||
