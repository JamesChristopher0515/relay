...
export function getAssignedJourneyProgressFromSingleStop({
  journey,
  assignedJourney,
  clientStop,
}: {
...
export default function getAssignedJourneyProgress({
  journey,
  assignedJourney,
  clientStops,
}: {
  ...
  const [milestone] = findId(
    journey.milestones,
    latestMilestoneClientStops[0].milestone
  )
  if (!milestone) {
    console.warn(
      `Couldn't find milestone ${latestMilestoneClientStops[0].milestone} in journey ${journey.name}`
    )
    return 0
  }

  // How many stops from the this milestone have we progressed past (skipping repeats)
  const latestMilestoneProgress =
    uniqBy(latestMilestoneClientStops, 'stop').filter(m => m.completedAt)
  ...
  return (
    milestonesCovered * ((mCount - 1) / mCount) +
