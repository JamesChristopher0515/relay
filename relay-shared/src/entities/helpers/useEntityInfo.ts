...
function clientShortName(client?: Client): string {
...
export default function useEntityInfo(entity: ClientEntityStr, id?: string) {
  ...
  invariant(
    !prevEntity.current || entity === prevEntity.current,
    `useEntityInfo entity cannot change at runtime. Previously: ${prevEntity.current} Now: ${entity}`
  )
  const queryHookWrapper = (queryHook, args?) =>
    queryHook(args ?? id, { skip: !id })?.data
  const queryHookManyWrapper = (queryHook, args) =>
    queryHook(args, { skip: !id })?.data?.data?.[0]

  /* eslint-disable react-hooks/rules-of-hooks */
  const clientMilestoneStop =
    entity !== 'client-milestone-stop'
      ? null
      : queryHookWrapper(useGetClientMilestoneStopQuery)

  const questionnaireResult =
    entity !== 'questionnaire-result'
      ? null
      : queryHookWrapper(useGetQuestionnaireResultQuery)

  const questionnaireResultQuestionnaire =
    entity !== 'questionnaire-result'
      ? null
      : queryHookWrapper(
          useGetQuestionnaireQuery,
  ...
  const clientMilestoneStopAssignedJourney =
    entity !== 'client-milestone-stop'
      ? null
      : queryHookWrapper(
          useGetAssignedJourneyQuery,
          clientMilestoneStop?.assignedJourney
        )

  const clientWorksheet =
    entity !== 'client-worksheet'
      ? null
      : queryHookWrapper(useGetClientWorksheetQuery)
  const clientWorksheetWorksheet =
    entity !== 'client-worksheet'
      ? null
      : queryHookWrapper(useGetWorksheetQuery, clientWorksheet?.worksheet)

  const clientMilestoneStopStop =
    clientMilestoneStopAssignedJourney && clientMilestoneStop
      ? findJourneyStop({
      ...
      : null
  const stopDeleted =
    clientMilestoneStopAssignedJourney &&
    clientMilestoneStop &&
  ...
  const info: {
    [K in EntityKey]: {
      ...
      canOpenInResourceManager?: boolean
      getShortName?: (doc: any) => string
      url?: string | ((doc: any) => string)
      hook?: any
      hookMultiple?: any
      plural?: string
    ...
    client: {
      name: 'Client',
      url: '/clients',
      getShortName: (doc: Client) => clientShortName(doc),
      icon: faCircle,
    ...
    category: {
      name: 'Category',
      getShortName: (doc: Category) => doc.name,
      icon: faCircle,
    ...
    'assigned-journey': {
      ...
      getShortName: (doc: AssignedJourney) => doc?.journey.name || '',
      // icon: JourneySvg,
      url: (assignedJourney) =>
        `/clients/${assignedJourney.client}/journeys/${assignedJourney._id}`,
    ...
    'client-worksheet': {
      ...
      getShortName: (doc: ClientWorksheet) =>
        clientWorksheetWorksheet?.name || '',
      url: (document) =>
        `/clients/${document.client}/worksheets/${document._id}`,
      icon: faFileExcel,
    ...
    'questionnaire-result': {
      ...
      icon: faClipboard,
      getShortName: (doc: QuestionnaireResult) =>
        questionnaireResultQuestionnaire?.name || '',
      url: (document) =>
        `/clients/${document.client}/questionnaires/${document.questionnaire}/${document._id}`,
