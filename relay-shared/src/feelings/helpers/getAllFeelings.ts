...
export function getFeelingColor(feeling: RelayFeeling) {
...
export function getFeelingsAverage(feelings: FeelingSelection[]) {
...
export function getFeelingName(feeling: RelayFeeling) {
  return capitalize(feeling.name)
}

export function getFeelingDescription(
  ...
  return (
    `Feeling "${startCase(firstFeeling.name)}" ${
      includeEmoji ? '' : ''
    }`.trim() + (howMany > 1 ? ` (${howMany} in time range)` : '')
  )
}

export function getFeelingByName(name: string) {
