import { useGetClientMilestoneStopQuery, useGetNotificationsQuery, useGetQuestionnaireResultQuery, } from '../../frontend/api/hooks/useApi';
...
export default function useAssignedJourneyWithInfo(assignedJourneyId, { includeActiveStopInfo } = { includeActiveStopInfo: false }) {
    ...
    const { data: questionnaireResult } = useGetQuestionnaireResultQuery(reviewQuestionnaireResultId, { skip: !reviewQuestionnaireResultId, ...qOptions });
    const { data: lastActiveClientStop } = useGetClientMilestoneStopQuery(assignedJourney?.activeClientStop ?? assignedJourney?.lastClientStop, { skip: !assignedJourney });
