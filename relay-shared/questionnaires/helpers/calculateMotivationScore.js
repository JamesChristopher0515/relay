...
export default function calculateMotivationScore(questionnaire, answers) {
    const allQuestions = getAllQuestionnaireQuestions(questionnaire, true);
    const statements = uniq(allQuestions.map((q) => q.id.replace(/_(happiness|importance)/, '')));
    return statements.reduce((accum, statement) => {
        ...
        const happinessAnswer = answers.find((answer) => idEq(answer.question, happinessQuestion._id));
        const importanceAnswer = answers.find((answer) => idEq(answer.question, importanceQuestion._id));
        assert(!!importanceAnswer && !!happinessAnswer, `Couldn't find answers for ${statement} in motivation questionnaire`);
        ...
        const negate = maxBy(happinessScale, 'value').value -
