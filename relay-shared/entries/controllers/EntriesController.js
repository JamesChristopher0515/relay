...
import { add, eachMonthOfInterval, endOfMonth, format, isSameDay, startOfMonth, } from 'date-fns';
...
export default makeController(function EntriesController(props) {
    const {} = props;
    const checkInToday = useTodayCheckIn();
    const [client] = useClientShared();
    const [loadStartDate, setLoadStartDate] = useState(startOfMonth(add(new Date(), { months: -2 })));
    const { data: entries, isLoading, error, } = wrapArrayHook(useGetCheckInsQuery({
    ...
    return {
        ...
        getInfoForDay: (day) => {
            ...
            const checkInsForDay = orderBy(checkIns?.filter(checkIn => isSameDay(checkIn.createdAt, day)) ?? [], ['createdAt'], ['asc']);
