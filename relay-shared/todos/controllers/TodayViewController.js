...
export default makeController(function TodayViewController({ day, noPoll, }) {
    const [client] = useClientShared();
    const { data: todayTodos, isLoading, refetch, } = wrapArrayHook(useGetTodayTodos({
        client: client._id,
        day,
    }, {}, noPoll));
    useEffect(() => {
    ...
    const todosByType = [
        ...
        noAssignedJourneyAndNotExpiredAppointment, getTodoType),
    ];
    return {
