...
import { useCreateMessageMutation, useGetMessagesQuery, } from '../../api/hooks/useApi';
export function ChatController({ practitioner, client, user, component: Component, unavailable, }) {
    ...
    const { data: result, isLoading, isFetching, } = useGetMessagesQuery(messagesQuery, {
        // Poll for changes on the first page only . When it changes, we remove all the other pages anyway
        pollingInterval: 3 * 1000,
    });
    const { data: pageResult, isLoading: pageIsLoading, isFetching: pageIsFetching, } = useGetMessagesQuery({ ...messagesQuery, page }, {
        skip: page === 0,
    });
    ...
    // changes over time
    const { canSend, ...rest } = useClassicReducer((prev) => {
        if (!prev) {
            prev = {};
        }
